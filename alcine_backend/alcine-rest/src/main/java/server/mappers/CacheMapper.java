...
public class CacheMapper {
	public static Cache getCache(String url, String r_location, String r_lang, int r_date, String tid, String fid) {
		...
		try {
			...
			ResultSet rs = DataBaseConect.exeQuery("SELECT * FROM \"CACHE\" WHERE url='"+ url +"' AND r_location='"+ r_location +"' AND r_lang='"+ r_lang +"' AND r_date="+ r_date + " AND fid='"+ fid + "' AND tid='"+ tid +"';");	
			while (rs.next()) {
